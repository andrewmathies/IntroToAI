...
class DTreeNode(Node):
    ...
    def get_entropy(self, labels):
        ...
        counts = np.zeros(len(uniqueElements))
        size = float(labels.size)

        for label in labels:
            for i in range(len(counts)):
                if label == uniqueElements.item(i):
                    counts[i] += 1

        entropy = 0.0

        for i in range(len(counts)):
            entropy -= float(counts.item(i))/size * np.log2(float(counts.item(i))/size)
